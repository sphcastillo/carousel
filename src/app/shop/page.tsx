...
export default async function ShopPage() {
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-20 md:px-8">
      <p className="kicker">01 / The boutique</p>
      <h1 className="display text-6xl md:text-8xl">Shop the carousel</h1>
      ...
      {collections && collections.length > 0 ? (
        <div className="mt-10 flex flex-wrap gap-3">
          {collections.map((collection) => (
            <a key={collection._id} href={`#${collection.slug}`} className="btn-round">
              {collection.title}
            </a>
          ))}
        </div>
      ...
      {collections && collections.length > 0 ? (
        collections.map((collection) => (
          <section key={collection._id} id={collection.slug || undefined} className="mt-20">
            <h2 className="display text-4xl md:text-5xl">{collection.title}</h2>
